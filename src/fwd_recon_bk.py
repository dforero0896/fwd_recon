...
from jax_powspec.correlations import powspec, powspec_vec, xi_vec, powspec_vec_fundamental, xi_vec_fundamental, bispec, compute_all_correlations, bispec
...
_, _, _, B_now, Q_now = bispec(delta_now, box_size, k1, k2, theta)
...
def loss(conv_kernel, delta_init):
    ...
    bispec_loss = 1e-14 * jnp.nanmean((B_now - B_ev)**2)
    
    return  pixelwise_loss + mono_k_loss + bispec_loss 
...
ax[10].imshow(new_conv_kernel.mean(axis=1), colorbar='right', norm='linear')
ax[11].imshow(new_conv_kernel.mean(axis=2), colorbar='right', norm='linear')
ax[8].format(title='Kernel')

ax[9].plot(theta, B_ev, label='Ev.', ls='--')
ax[9].legend()
fig.savefig("plots/fwd_recon_bk.pdf", dpi=300)