...
#@jax.jit
conv_kernel_dims = (2,2,2,1,1)
dn = jax.lax.conv_dimension_numbers(delta_init[None,...,None].shape, conv_kernel_dims,
                                ('NHWDC', 'HWDIO', 'NHWDC'))
def loss(params, delta_init):
    conv_kernel, w, b = params
    init = w * jax.lax.conv_general_dilated(delta_init[None, ..., None], conv_kernel, (1,1,1), 'SAME', (1,1,1), (1,1,1), dn).squeeze() + b
...
conv_kernel = jax.random.uniform(subkey, conv_kernel_dims)

opt_state = opt_init((conv_kernel, 1., 0.))
...
new_conv_kernel, w, b = get_params(opt_state)
delta_init = w * jax.lax.conv_general_dilated(delta_init[None, ..., None], new_conv_kernel, (1,1,1), 'SAME', (1,1,1), (1,1,1), dn).squeeze() + b
k, pk_init, _ = powspec_vec(delta_init, box_size, k_edges)
s, xi_init, _ = xi_vec(delta_init, box_size, s_edges)
delta_ev = bias * evolve_lagrangian(pos_lagrangian, delta_init, growth_factor)
k, pk_ev, _ = powspec_vec(delta_ev, box_size, k_edges)
