...
knorm_1d = jnp.fft.fftfreq(n_bins, d=box_size/n_bins) * 2 * jnp.pi
knorm = jnp.sqrt(knorm_1d[:,None,None]**2 + knorm_1d[None,:,None]**2 + knorm_1d[None,None,:n_bins//2+1]**2)
knorm_1d = jnp.fft.rfftfreq(n_bins, d=box_size/n_bins) * 2 * jnp.pi
...
def loss(conv_kernel, delta_init):
    #_conv_kernel = jnp.interp(knorm.flatten(), knorm_1d[:n_bins//2+1], conv_kernel).reshape((n_bins, n_bins, n_bins//2+1))
...
#conv_kernel = jnp.ones((n_bins//2 + 1))
...
print(knorm_1d[:n_bins//2+1])
print(knorm.flatten())
new_conv_kernel = get_params(opt_state)
#new_conv_kernel = jnp.interp(knorm.flatten(), knorm_1d[:n_bins//2+1], new_conv_kernel).reshape((n_bins, n_bins, n_bins//2+1))
...
ax[6].legend(loc='top')
ax[7].legend(loc='top')
ax[8].imshow(new_conv_kernel.mean(axis=0), colorbar='right', norm='linear')
ax[9].imshow(new_conv_kernel.mean(axis=1), colorbar='right', norm='linear')
ax[10].imshow(new_conv_kernel.mean(axis=2), colorbar='right', norm='linear')

kernel_hist, _ = jnp.histogram(knorm, bins= k_edges, weights = new_conv_kernel)
norm_hist, _ = jnp.histogram(knorm, bins= k_edges, weights = (new_conv_kernel != 0.).astype(jnp.float32))

ax[11].plot(k, kernel_hist / norm_hist , label='0', lw=0, marker='*')


ax[11].legend()
ax[8].format(title='Kernel')
fig.savefig("plots/fwd_recon_small.pdf", dpi=300)