...
# Build initial condition tracers as lognormal mock
init_positions = jax.device_put(np.load("data/data/lognormal_ic_tracer.npy"))
delta_init = jnp.zeros((n_bins, n_bins, n_bins))
delta_init = cic_mas_vec(delta_init,
                init_positions[:,0], init_positions[:,1], init_positions[:,2], jnp.broadcast_to([1], init_positions.shape[0]), 
                init_positions.shape[0], 
                0., 0., 0.,
                box_size,
                n_bins,
                True)
...
#fig.savefig("plots/fwd_recon.pdf", dpi=300); exit()
#@jax.jit
def loss(init_positions):
...
init_positions = get_params(opt_state)
delta_init = jnp.zeros((n_bins, n_bins, n_bins))
delta_init = cic_mas_vec(delta_init,
                init_positions[:,0], init_positions[:,1], init_positions[:,2], jnp.broadcast_to([1], init_positions.shape[0]), 
                init_positions.shape[0], 
